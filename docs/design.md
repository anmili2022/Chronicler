...
- `FloatingStatusWindow`：轻量显示当前新月岛史官目标（含掉落标记），并提供导航、清除导航、回营地、全自动开关、待命点和效率按钮。目标列表使用紧凑的 `[FATE]` / `[CE]` 淡蓝标签作为各区第一条目标前缀，避免额外分区行；CE 的特殊灵魂碎晶标签为惨白魔人 `[青]`、亡灵法师 `[亡]`，标签颜色分别为青色和橙红色，悬停显示掉落说明。开启成就悬浮窗联动后，送死分区提供“开启送死”/“关闭送死”切换按钮，并将其放在“送死坐标”按钮前。
...
- 成就页签按“显示与联动”“复活后行为”“悬浮窗成就提示”“送死坐标”“救人成就进度”“调试工具”分组展示；操作说明主要通过控件 tooltip 提供，页面只保留必要状态和进度。
- 成就页签的“送死模式”开启后，复活会等待角色与 vnavmesh 就绪，随后通过 `vnavmesh.Path.MoveTo` 直接步行到送死坐标；不再使用救援坐标，不自动寻路、上坐骑或使用水晶传送。悬浮窗标题显示与全自动相同风格的“送死”状态标签。
