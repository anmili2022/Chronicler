...
- `自动寻路`：全自动导航设置。
- `设置`：通用选项与地图 ID 设置。
- `DEBUG`：显示调试区、导航调试开关、显示当前位置/新月岛史官目标距离、复制全部调试信息，以及调试区内容。
...
10. 如果需要岛内传送，优先使用玩家水平距离 40 码内最近的水晶；附近没有水晶时才使用亚返回，并在回营地后前往营地大水晶传送。
...
- 新月岛内从营地点击目标导航时，不再比较“当前点到目标”的距离收益；只要目标推荐水晶不是当前营地水晶，就进入岛内传送流程。
- 多路线导航的首个航点按最终 Boss 目标选择传送点，避免首航点的距离判断覆盖已经确定的营地传送流程。
- 决定使用岛内传送后，若玩家水平距离 40 码内有传送水晶，直接步行到该水晶传送；附近没有水晶时才使用亚返回前往营地水晶。
- 附近水晶链路是“到 40 码内最近水晶 -> 进入水晶 8 码范围 -> 稳定等待 2 秒 -> Lifestream 传送 -> 继续步行导航”。
- 无附近水晶时的回营地链路是“亚返回 -> 到营地大水晶 -> 进入水晶 8 码范围 -> 稳定等待 2 秒 -> Lifestream 传送 -> 继续步行导航”，避免在营地落点附近直接步行到目标。
