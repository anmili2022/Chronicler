...
- `FloatingStatusWindow`：轻量显示当前新月岛史官目标（含掉落标记），并提供导航、清除导航、回营地、全自动开关、待命点和效率按钮。目标列表使用紧凑的 `[FATE]` / `[CE]` 淡蓝标签作为各区第一条目标前缀，避免额外分区行；CE 的特殊灵魂碎晶标签为惨白魔人 `[青]`、亡灵法师 `[亡]`，标签颜色分别为青色和橙红色，悬停显示掉落说明。开启成就悬浮窗联动后，送死分区提供“开启送死”/“关闭送死”切换按钮，并将其放在“送死坐标”按钮前；救人成就提示只显示当前地图对应的船医或名医进度。
...
- 实时宝箱探测、计数和接近提示采用双重校验：先按当前南/北岛固定实时 `BaseId` 判断铜/银类型，再通过 `Treasure.SGB.RowId` 复核模型。个人或非地图战利品箱 `1996`、`2000-2003` 即使复用铜/银模型，也不在实时 `BaseId` 白名单内而被排除。
- 已核验实时对象 ID：南征 `1792` 为银、`1853` 为铜；北征 `2006` 为银、`2019` 为铜。新增实时宝箱对象 ID 时需更新 `ChestCatalog.GetLiveTreasureLabel`，否则不会被实时探测。
...
- 悬浮窗中的 `[笔]` 标签支持悬停提示，显示对应调查笔记编号和来源；若调查笔记联动开启且该笔记已解锁，则标签隐藏。

## 宝箱页签

- 宝箱页签按当前排序结果以三列布局显示南征/北征内置宝箱坐标。
- 三列布局不改变按宝箱 ID、距离或 BOCCHIOK 推荐路线排序的行为；每个条目的导航和 Flag 操作保持不变。
...
5. 新目标出现后默认等待 5 秒；到期时目标仍有效才导航。
   - 高优先级目标抢占当前目标时，延迟期间继续原导航；到期后先停止原目标的步行、传送、路线和下坐骑状态，再启动新目标导航。
...
- 成就页签按“显示与联动”“复活后行为”“悬浮窗成就提示”“送死坐标”“成就进度”“调试工具”分组展示；操作说明主要通过控件 tooltip 提供，页面只保留必要状态和进度。
- 成就页签的“送死模式”默认关闭，且与“目标 BaseId”输入框位于同一行。开启后，复活会等待角色与 vnavmesh 就绪，随后通过 `vnavmesh.Path.MoveTo` 直接步行到送死坐标；不再使用救援坐标，不自动寻路、上坐骑或使用水晶传送。悬浮窗标题显示与全自动相同风格的“送死”状态标签。
- 悬浮窗“开启送死”/“关闭送死”按钮直接切换并保存 `AchievementRespawnNavigationEnabled`；关闭时清理等待中的复活导航、当前导航重试和到达后的目标选择状态，不再继续重试、选中目标或释放技能。
- 成就页签的“目标 BaseId”设为非 `0` 时，复活导航进入记录坐标 8y 内后会选中最近的同 `BaseId` 可选中对象；45 秒内找不到对象会输出失败提示。
- 成就目标成功选中后会立即按当前职业释放一次固定远程技能：赤魔法师冲击（`16526`）、黑魔法师火炎（`141`）、召唤师毁荡（`3579`）、绘灵法师火炎之红（`34650`）、白魔法师飞石（`119`）、贤者注药（`24283`）、学者极炎法（`25865`）、占星术士凶星（`3596`）、机工士热分裂弹（`7412`）、吟游诗人强力射击（`97`）、舞者瀑泻（`15989`）。其他职业仅选中目标，不释放技能。
- 成就进度服务读取南岛三角区·船医3（ID `3674`）、北岛三角区·名医3（ID `4016`）和过路圣人（ID `1357`）。过路圣人仅在成就页签显示，不进入悬浮窗。
- 登录或切换角色后，成就进度服务会重置旧角色缓存，并仅自动执行一次 `/achievement` 触发客户端加载；未收到进度前统一显示“读取中”，避免显示无效的 `0 / 0`。
- 成就页签 DEBUG 的“扫描成就候选 ID”会列出名称包含“船医”、“名医”或“过路圣人”的 ID，便于核验游戏数据表。
...
- 自动目标开始时先输出 `[全自动] 导航到 FATE/CE ...`，随后以事件原始中心点执行第一次 `ShouldTeleportToTarget` 预判。预判为 `false` 时不解析 Boss 路线，直接选择随机偏移目标或原始中心点；预判为 `true` 时才解析并优先使用有效的 Boss 路线。
- 单点导航先在事件中心的配置半径内选择随机目标，再使用 vnavmesh `NearestPoint` 将其吸附到导航网格。若本次随机启用了到达后下坐骑，此时登记吸附后的最终目标并输出 `[导航通知] 下坐骑: 已设置目标点 (...)`；该消息仅表示角色进入目标水平距离 10 码内后会尝试下坐骑，不表示已经下坐骑，也不表示传送判定已经结束。
- `NavigateToInternal` 登记下坐骑目标后，仅在 Lifestream 岛内传送 IPC 可用时，以吸附后的单点目标再次执行 `ShouldTeleportToTarget`。第二次判定为 `true` 时进入水晶传送链路，为 `false` 或 IPC 不可用时调用 vnavmesh 直接步行。因此正常日志顺序可以是“导航到目标 -> 下坐骑目标设定 -> 传送判定/步行”，后两步的诊断信息只有开启`导航调试`才显示。
- 两次 `ShouldTeleportToTarget` 使用的目标可能不同：第一次使用事件原始中心点，第二次使用随机偏移并吸附后的单点目标。边界情况下两次结果可能不同；最终是否传送以 `NavigateToInternal` 中的第二次判定和 IPC 可用性为准。
- `ShouldTeleportToTarget` 使用角色实时位置而非上一个导航目标的位置。玩家水平距离 60 码内有水晶时，以最近水晶为锚，只要“目标推荐水晶 ID != 锚水晶 ID”便选择传送；两者相同时直接步行。附近没有水晶时，使用“玩家到目标三维距离 + 传送阈值 > 目标水晶到目标三维距离”判断传送是否更划算。
...
- 条件使用“或”语义：角色位于当前地图营地时，任一已启用的条件满足便停止当前导航并执行 `/pdr leaveduty`；待命点及岛内其他区域不会触发离岛，两项都未勾选时也不会自动离岛。
